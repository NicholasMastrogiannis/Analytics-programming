...
## Task 4 Write the code to investigate the characteristics of patients (confirmed or suspected) who recover from COVID-19 compared to those who don’t. 
...
```{r}
# Identify COVID-related conditions 
covid_conditions <- conditions_data %>%
  filter(grepl("COVID|SARS|coronavirus", DESCRIPTION, ignore.case = TRUE))

# Determine recovery status 
# Patients recovered if 'Stop' date recorded and no death date
covid_status <- covid_conditions %>%
  group_by(PATIENT) %>%
  summarise(
    covid_start = min(as.Date(START)),
    covid_stop = min(as.Date(STOP), na.rm = TRUE),
    recovered = ifelse(!is.na(covid_stop), 1, 0) )


# Merge with patient demographics
covid_data <- covid_status %>%
  inner_join(patient_data, by = c("PATIENT" = "Id")) %>%
  mutate(
    age = as.numeric(floor((Sys.Date() - as.Date(BIRTHDATE)) / 365.25)),
    time_to_recovery = as.numeric(as.Date(covid_stop) - as.Date(covid_start))
  )
...
covid_summary <- covid_data %>%
  ...
  summarise(
    ...
    male_percent = mean(GENDER == "M", na.rm = TRUE) * 100,
    avg_time_to_recovery = mean(time_to_recovery, na.rm = TRUE)
  )
...
```{r}
# Visualise age vs recovery status 
ggplot(covid_data, aes(x = factor(recovered), y = age, fill = factor(recovered))) +
  geom_boxplot() +
  labs(
    title = "Age Comparison: Recovered vs Not Recovered COVID Patients",
    x = "Recovered (1 = Yes, 0 = No)",
    y = "Age"
  ) +
  theme_minimal()

# Visualise time to recovery 
ggplot(filter(covid_data, recovered == 1), aes(x = time_to_recovery)) +
  geom_histogram(bins = 25, fill = "steelblue") +
  labs(
    title = "Distribution of Time to Recovery",
    x = "Days from Diagnosis to Recovery",
    y = "Number of Patients"
  ) +
  theme_minimal()
```
### Analysis on Task 4 findings.
```{r}
# The results show that age and gender seem to play an important role in how people recover from COVID-19. Older patients tended to take longer to recover or were less likely to fully recover, while younger people generally recovered faster. There were also small differences between men and women, with men showing slightly slower recovery rates. Some variation across zip codes was also seen, which could be linked to differences in healthcare access or local conditions. Overall, the analysis suggests that factors like age, gender, and where someone lives can all influence how quickly they recover from COVID-19.
```
